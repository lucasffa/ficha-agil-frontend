...
export interface Ficha {
  ...
  ComposicaoFamiliar: {
    ...
    Profissao: string;
    IdSitTrabalhista: number | null;
    IdEscolaridade: number | null;
    Renda: number | null;
  }[];
  Despesas: {
...
export function CandidatoFicha() {
  ...
  async function postFicha(ficha: Ficha) {
    ...
    ficha.OutrasFichasGrupoFamiliar.forEach((element, key) => {
      ...
      dataForm.append(
        `GRUPOFAMILIAR[${key}].NomeCompletoFamiliar`,
        element.NomeCompletoFamiliar?.toString() ?? ''
      );
    });
